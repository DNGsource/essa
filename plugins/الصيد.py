...
from essa  import senzir 

# A special file for senzir Telethon fishing for you
# needs
ua = UserAgent()
# ON WATCH
USERNAMES = []
...
@essa .on(events.NewMessage(outgoing=True, pattern=r'.اغلاق الصيد'))
...
@essa .on(events.NewMessage(outgoing=True, pattern=r'.حالة الصيد'))
async def StopPhishingHundler(event):
    global workers
...
@essa .on(events.NewMessage(outgoing=True, pattern=r'.صيد ?(.*)'))
async def PhishingHundler(event):
    ...
    if PhishType not in valid_types:
        ...
        if workers.get(PhishType).get('worker') == False:
            ...
            try:
                channel = await event.client(CreateChannelRequest(title=f'صيد : {PhishType}', about=f'essa '))
...
@essa .on(events.NewMessage(outgoing=True, pattern=r'.سحب ?(.*)'))
async def CheckAndApply(event):
    ...
    # حقوق سينزر تخمط بدون ذكر المصدر اهينك
    RUN = await CheckUsername(event, username, event.sender_id)
    if RUN == True:
        RUN = await UpdateUsername(event, username)

# USERNAMES UNCHECK
@essa .on(events.NewMessage(outgoing=True, pattern=r'.اغلاق سحب ?(.*)'))
...
async def UpdateUsername(event, user):
    ...
    # # حقوق سينزر تخمط بدون ذكر المصدر اهنك 
