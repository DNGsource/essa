...
async def pyZip(e):
    ...
    zip_ = f"{nem_}.zip" if nem_ else "ESSA _Zip.zip"
    password = pass_ if pass_ else "ESSA "
...
async def iq(event):
    ...
    senzir  = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
    ...
    async for user in event.client.iter_participants(senzir .full_chat.id):
        try:
...
async def _(event):
    ...
    senzir event = await edit_or_reply(event, "**✘ ⦙  جاري تفليش مجموعتك أنتظر قليلآ 🚮**")
...
async def tmuter(event):  # sourcery no-metrics
    ...
    if not user:
        return
    if not reason:
        return await catevent.edit("**✘ ⦙ رجاء طريقه كتابه الامر خاطئه قم بروئيه قناه شروحات الاوامر : @E_I_I7**")
...
async def mentionall(event):
  ...
  if mode == "text_on_cmd":
    ...
    usrnum = 0
    usrtxt = ""
    async for usr in senzir .iter_participants(event.chat_id):
      usrnum += 1
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await senzir .send_message(event.chat_id, f"{usrtxt}\n\n{msg}")
...
@senzir .on(admin_cmd(pattern=f"{UNBANDD}(?: |$)(.*)"))
