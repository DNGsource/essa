...
EMOJI_SEN = [    "يمكنك إدراج عدة رموز تعبيرية في رسالة واحدة ، ولكني أوصي باستخدام ما لا يزيد عن اثنين لكل ملصق" ,].  # CODE BY : T.me/IC_X_K
...
def pack_name(userid, pack, is_anim):
    if is_anim:
        return f"essabot_{userid}_{pack}_anim"
    return f"essabot_{userid}_{pack}"
def char_is_emoji(character):
    return character in catemoji.UNICODE_EMOJI["en"] #code by t.me/IC_X_K
...
async def add_to_pack(    catevent,    conv,    args,    packname,    pack,    userid,    username,    is_anim,    stfile,    emoji,    cmd,    pkang=False ):
    ...
    return pack, packname
@senzir.on(admin_cmd(pattern="صنع ملصق(?:\s|$)([\s\S]*)"))    
async def kang(args):  
    photo = None
    emojibypass = False
    is_anim = False
...
@senzir.on(admin_cmd(pattern="جلب الملصقات(?:\s|$)([\s\S]*)"))    
...
@senzir.on(admin_cmd(pattern="انشاء حزمه ملصقات(?:\s|$)([\s\S]*)"))   
async def pic2packcmd(event):
    ...
    try:
        ...
        emoji = "▫️"
    chat = "@Stickers"
    name = "essabot_" + "".join(        random.choice(list(string.ascii_lowercase + string.ascii_uppercase))
        for _ in range(16)    )
...
@essa.on(admin_cmd(pattern="جلب معلومات الملصق$"))    
...
@essa.on(admin_cmd(pattern="ملصق ?([\s\S]*)"))    
async def cb_sticker(event):
    ...
    for pack in results:
        if pack.button:
            packtitle = (pack.find("div", "sticker-pack__title")).get_text() #code by t.me/IC_X_K
