...
new_essa = TelegramClient(StringSession(Config.STRING_SESSION), Config.APP_ID, Config.API_HASH)

async def PyStart():
    global essa_py
    try:
        await new_essa.start()
        essa_py = PyTgCalls(new_essa) 
        await essa_py.start()
...
async def LeaveStream(chat_id):
    global essa_py
    await essa_py.leave_group_call(
        chat_id,
    )


# DOWNLOAD THEN STREAM AUDIO
@ESSA.on(events.NewMessage(outgoing=True, pattern=r'.تشغيل صوتي'))
...
@ESSA.on(events.NewMessage(outgoing=True, pattern=r'.تشغيل فيديو'))
...
# LEAVE STREAM
@ESSA.on(events.NewMessage(outgoing=True, pattern=r'.اغلاق البث'))
