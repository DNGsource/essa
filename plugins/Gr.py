...
from essa.utils import admin_cmd, sudo_cmd, eor
...
@ESSA.on(admin_cmd(pattern=r"المحظورين?(.*)"))
...
@ESSA.on(admin_cmd(pattern="تشفير الملف ?(.*)"))
async def pyZip(e):
    if e.fwd_from:
        return
    ...
    zip_ = f"{nem_}.zip" if nem_ else "senzir_Zip.zip"
    password = pass_ if pass_ else "senzir"
...
@ESSA.on(admin_cmd(pattern=f"{MUTE}(?:\s|$)([\s\S]*)"))
async def muteessa(event):
    if event.is_private:
        ...
        if event.chat_id == essa.uid:
            return await edit_delete(event, "**- لا تستطــع كتـم نفسـك**")
        if event.chat_id == 6335191266:
...
@ESSA.on(admin_cmd(pattern=f"{UNMUTE}(?:\s|$)([\s\S]*)"))
async def unmutessa(event):
    if event.is_private:
        replied_user = await event.client.get_entity(event.chat_id)
...
@ESSA.on(admin_cmd(pattern=r"المشرفين(?: |$)(.*)"))
async def _(event):
...
@ESSA.on(admin_cmd(pattern=r"البوتات?(.*)"))
async def _(event):
...
@ESSA.on(admin_cmd(pattern=r"الأعضاء(?: |$)(.*)"))
...
@ESSA.on(admin_cmd(pattern=r"معلومات(?: |$)(.*)"))
...
@ESSA.on(admin_cmd(pattern=f"{addition} ?(.*)"))
async def iq(event):
    ...
    async for user in event.client.iter_participants(ESSA.full_chat.id):
        try:
...
@ESSA.on(admin_cmd(pattern=f"{TFLASH}(.*)"))
async def _(event):
    ...
    essaevent = await edit_or_reply(event, "**✘ ⦙  جاري تفليش مجموعتك أنتظر قليلآ 🚮**")
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            LOGS.info(str(e))
    await ESSA event.edit(f"**✘ ⦙   تم بنجاح تفليش مجموعتك من {total} الاعضاء 🚮**")
    
async def ban_user(chat_id, i, rights):
    try:
        await ESSA(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@ESSA.on(admin_cmd(pattern=f"{LEFT}(.*)"))
async def kickme(leave):
    await leave.edit("**✘ ⦙   جـاري مـغادرة المجـموعة مـع السـلامة  🚶‍♂️  ..**")
    await leave.client.kick_participant(leave.chat_id, "me")

@ESSA.on(admin_cmd(pattern=f"{REMOVEBAN}(.*)"))
async def _(event):
...
@ESSA.on(admin_cmd(pattern=f"المحذوفين ?([\s\S]*)"))
...
@ESSA.on(admin_cmd(pattern=r"احصائيات الاعضاء ?([\s\S]*)"))
...
@ESSA.on(admin_cmd(pattern="تقيد(?:\s|$)([\s\S]*)",))
...
@ESSA.iq_cmd(pattern="اكتم(?:\s|$)([\s\S]*)",)
...
@ESSA.on(admin_cmd(pattern=r"احظر(?:\s|$)([\s\S]*)",))
...
@ESSA.on(admin_cmd(pattern=r"حظر عام(?:\s|$)([\s\S]*)",))
...
@ESSA.on(admin_cmd(pattern=r"الغاء حظر العام(?:\s|$)([\s\S]*)",))
...
@ESSA.on(admin_cmd(pattern=r"المحظورين عام$",))
async def gablist(event):
    gbanned_users = gban_sql.get_all_gbanned()
    GBANNED_LIST = "Current Gbanned Users\n"
    if len(gbanned_users) > 0:
...
@ESSA.on(admin_cmd(pattern=r"معلومات تخزين المجموعه(?:\s|$)([\s\S]*)"))
async def _(event):  # sourcery no-metrics
    reply = await event.get_reply_message()
    input_str = event.pattern_match.group(1)
    ...
    media_dict = {
        m: {"file_essa": 0, "count": 0, "max_size": 0, "max_file_link": ""}
        for m in TYPES
    }
    async for message in event.client.iter_messages(
        ...
        if media is not None:
            ...
            if message.file.size > media_dict[media]["max_essa"]:
                media_dict[media]["max_essa"] = message.file.size
                if type(chatdata).__name__ == "Channel":
                    media_dict[media][
                        "max_file_link"
    ...
    for mediax in TYPES:
        x.add_row(
            [
                ...
                humanbytes(media_dict[mediax]["file_essa"]),
            ]
        )
...
@ESSA.iq_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@ESSA.iq_cmd(incoming=True, func=lambda e: e.mentioned, edited=False, forword=None)
...
@ESSA.on(admin_cmd(pattern=r"تخزين الخاص (تشغيل|ايقاف)$"))
...
@ESSA.on(admin_cmd(pattern=r"تخزين الكروبات (تشغيل|ايقاف)$"))
...
@ESSA.on(admin_cmd(pattern=f"{LINKK} ?(.*)"))
async def iq(SLQ):
...
@ESSA.on(admin_cmd(pattern="عدد رسائلي ?(.*)"))
async def iq(SLQ):
...
@ESSA.on(admin_cmd(pattern="تغير صورة( المجموعة| -d)$"))
...
@ESSA.on(admin_cmd(pattern=f"{ADMINRAISE}(?: |$)(.*)"))
...
@ESSA.on(admin_cmd(pattern=f"{UNADMINRAISE}(?: |$)(.*)"))
...
@ESSA.on(admin_cmd(pattern="تثبيت(?: |$)(.*)"))
async def pin(event):
    "✘ ⦙   تثبيت  📌"
...
@ESSA.on(admin_cmd(pattern="الغاء التثبيت(?: |$)(.*)"))
async def pin(event):
...
@ESSA.on(admin_cmd(pattern="جلب الاحداث(?: |$)(.*)"))
...
moment_worker = []
@ESSA.on(admin_cmd(outgoing=True, pattern="all?(.*)"))
async def mentionall(event):
  ...
  if mode == "text_on_cmd":
    ...
    usrnum = 0
    usrtxt = ""
    async for usr in essa.iter_participants(event.chat_id):
      usrnum += 1
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await ESSA.send_message(event.chat_id, f"{usrtxt}\n\n{msg}")
  ...
  if mode == "text_on_reply":
    ...
    async for usr in essa.iter_participants(event.chat_id):
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await ESSA.send_message(event.chat_id, usrtxt, reply_to=msg)
        await asyncio.sleep(2)
        usrnum = 0
        usrtxt = ""

@ESSA.on(admin_cmd(outgoing=True, pattern="ايقاف التاك?(.*)"))
...
@ESSA.on(admin_cmd(pattern=f"{BANDD}(?: |$)(.*)"))
async def _ban_person(event):
    "✘ ⦙   لحـظر شخص في كـروب مـعين"
    user, reason = await get_user_from_event(event)
    if not user:
        return
    if user.id == 6335191266:
...
@ESSA.on(admin_cmd(pattern=r"zip(?:\s|$)([\s\S]*)"))
...
@ESSA.on(admin_cmd(pattern=r"unzip(?:\s|$)([\s\S]*)"))
...
@ESSA.on(admin_cmd(pattern="معرفه(?: |$)(.*)"))
...
@ESSA.on(admin_cmd(pattern="دعوه للمكالمه(?: |$)(.*)"))
async def _(e):
...
@ESSA.on(admin_cmd(pattern="بدء مكالمه(?: |$)(.*)"))
async def _(e):
    try:
...
@ESSA.on(admin_cmd(pattern=f"{UNBANDD}(?: |$)(.*)"))
...
async def watcher(event):
    if is_muted(event.sender_id, event.chat_id):
        try:
            ...
            LOGS.info(str(e))
@ESSA.on(admin_cmd(pattern="صنع (مجموعه|قناه) (.*)"))
async def iq(event):
...
@ESSA.on(admin_cmd(pattern="تلقائي ?(.*)"))
async def _(event):
...
@ESSA.on(admin_cmd(pattern="الغاء_التلقائي ?(.*)"))
async def _(event):
    if (event.is_private or event.is_group):
        return await edit_or_reply(event, "امر التلقائي يستخدم فقط للقنوات.")
    hel_ = event.pattern_match.group(1)
...
@ESSA.on(admin_cmd(pattern=r"جلب الصور(?:\s|$)([\s\S]*)"))
async def potocmd(event):
    ...
    if uid.strip() == "":
        ...
        await event.client.send_file(event.chat_id, send_photos)
    await event.delete()
@ESSA.on(admin_cmd(pattern="تفعيل ([\s\S]*)"))    
async def _(event):  
    input_str = event.pattern_match.group(1)
    ...
    cat = base64.b64dexcode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
...
@ESSA.on(admin_cmd(pattern="تعطيل ([\s\S]*)"))    
async def _(event):  
    input_str = event.pattern_match.group(1)
    peer_id = event.chat_id
    if not event.is_group:
...
@ESSA.on(admin_cmd(pattern="صلاحيات المجموعه$"))    
async def _(event):  
    res = ""
