...
@essa .iq_cmd(pattern="اغلاق تعديل الميديا")
async def cleanup_command(event):
    if await is_admin(event.client, event.chat_id, event.sender_id):
        global handler

        @essa .on(events.MessageEdited(incoming=True, chats=event.chat_id))
...
@essa .iq_cmd(pattern="فتح تعديل الميديا")
async def stop_cleanup_command(event):
    if await is_admin(event.client, event.chat_id, event.sender_id):
        essa .remove_event_handler(handler)
        await event.edit("- تم اغلاق نظام حذف الميديا المعدلة.")
    else:
        await event.edit("- ليس لديك الصلاحيات الكافية لأستخدام هذا الامر.")                               
                        
@essa .iq_cmd(pattern="فتح الزخرفة الانجليزية")
async def zakrafaon(event):
    ...
    if gvarstatus("enzakrafa"):
        ...
        return
@essa .iq_cmd(pattern="اغلاق الزخرفة الانجليزية")
async def zakrafaoff(event):
    ...
    if gvarstatus("enzakrafa"):
        ...
        return
@essa .on(events.NewMessage(outgoing=True))
...
@essa .iq_cmd(pattern="انشاء ?(.*)")
async def inshai(event):
    msg = event.text.split()
    username = msg[1]
    chat = "@creationdatebot"
    response = await senzir .send_message("creationdatebot", f"/id {username}")
