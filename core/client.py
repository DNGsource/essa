        ...
        **kwargs,
        ...
        def decorator(func):  # sourcery no-metrics
            async def wrapper(check):
                ...
                try:
                    ...
                    if not disable_errors:
                        ...
                        text += f" عذرا قم بكتابه الأمر بشكل صحيح راجع : @essa ** "
                        await check.client.send_message(
                            Config.PRIVATE_GROUP_BOT_API_ID, text, link_preview=False
                        )

            from .session import senzir
            ...
            if pattern is not None:
                ...
                if edited:
                    essa.add_event_handler(func, events.MessageEdited(**kwargs))
                essa.add_event_handler(func, events.NewMessage(**kwargs))
            return wrapper
    ...
    def bot_cmd(
        ...
        **kwargs,
        ...
        def decorator(func):
            ...
            from .sessaon import essa 

            if edited is True:
                essa.tgbot.add_event_handler(func, events.MessageEdited(**kwargs))
            else:
                essa.tgbot.add_event_handler(func, events.NewMessage(**kwargs))
