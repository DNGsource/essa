...
from IqArab import senzir, catversion, StartTime
...
async def inline_handler(event):  
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**senzirbot"):
            ...
            builder = event.builder
            secret = os.path.join("./senzir", "secrets.txt")
            try:
        ...
        await event.answer([result] if result else None)
@senzir.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@senzir.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
