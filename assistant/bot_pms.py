...
async def bot_start(event):
    ...
    if chat.id != Config.OWNER_ID:
        ...
        if customstrmsg is not None:
            ...
            start_msg = f"**⌔︙ اهلا وسهلا عزيزي  {mention} **\
                        \n**⌔︙ {my_mention}'انا البوت المساعد .**\
                        \n**⌔︙ يمكنك التواصل مع صاحب البوت فقط ارسل رسالتك 👤.**\
                        \n\n**⌔︙ البوت خاص بسورس :** [Source Rizer](t.me/GM_DU)"
        buttons = [
            (
                Button.url("تنصيب سورسنا", "t.me/E_I_I7"),
                Button.url(
                    "قناه المساعد",
                    "https://t.me/HG_STF",
                ),
        ...
        start_msg = ESSA "**⌔︙ اهلا وسهلا بك ايها المطور ⚜️**\
...
@ESSA.bot_cmd(incoming=True, func=lambda e: e.is_private)
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await ESSA.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await ESSA.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await ESSA.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await ESSA.tgbot.get_entity(BOTLOG_CHATID)
            await ESSA.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await ESSA.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@ESSA.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await ESSA.get_entity(user_id)
    except Exception as e:
...
@ESSA.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@ESSA.bot_cmd(incoming=True, func=lambda e: e.is_private)
@ESSA.bot_cmd(edited=True, func=lambda e: e.is_private)
